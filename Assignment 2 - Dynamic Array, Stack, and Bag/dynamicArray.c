...
{	
    ...
    assert(v != NULL);
    TYPE *originalData;
    int originalCapacity = v->size;
    originalData = v->data;
    
    /* Create a new dyn array with larger underlying array */
    initDynArr(v, newCap);
    for(i = 0; i < originalCapacity; i++)
    {
        v->data[i] = originalData[i];
    }
    
    v->size = originalCapacity;
...
void swapDynArr(DynArr *v, int i, int  j)
{
    ...
    TYPE  temp;
    assert(v != NULL && v->size > 0);
    assert(i < v->size && i >= 0 && j < v->size && j >= 0);
